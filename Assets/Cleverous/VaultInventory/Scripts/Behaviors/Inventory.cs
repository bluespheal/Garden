...
namespace Cleverous.VaultInventory
{
    ...
    public partial class Inventory : NetworkBehaviour
    {
        ...
        public virtual void Initialize(IUseInventory owner, InventoryState state)
        {
            ...
            Configuration = (InventoryConfig)Vault.Get(state.ConfigIndex);
            Content = new List<RootItemStack>();

            for (int i = 0; i < state.ItemIndexes.Count; i++)
            {
                if (state.ItemIndexes[i] == -1)
                {
                    Content.Add(null);
                    continue;
                }
                ...
                Content.Add(new RootItemStack(
                    (RootItem)Vault.Get(state.ItemIndexes[i]),
        ...
        [Server]
        public virtual int DoTake(string itemTitle, int amountToRemove)
        {
            if (!IsInitialized) return -1;
        ...
        public virtual int DoTake(int inventoryIndex, int amountToRemove)
        {
            ...
            if (amountToRemove < Content[inventoryIndex].StackSize)
            {
                string itemTitle = Content[inventoryIndex].Source.Title;
                int vaultItemId = Vault.GetIndex(itemTitle);
                ...
                RpcRemoteUpdateSlot(inventoryIndex, vaultItemId, Content[inventoryIndex].StackSize, false);
                return 0;
            }
        ...
        [Server]
        public virtual int DoCount(string itemTitle)
        {
            if (!IsInitialized) return -1;
        ...
        [ClientRpc]
        public virtual void RpcRemoteUpdateSlot(int index, int itemId, int amount, bool flagAddRemove)
        {
            ...
            RootItemStack newData = new RootItemStack((RootItem)Vault.Get(itemId), amount);
